...
class ProductsController extends BaseController
{
        ...
        if ($ext != 'csv') {
            ...
            if (($handle = fopen($file->getPathname(), "r")) !== false) {
                ...
                foreach ($header as $key => $value) {
                    ...
                    if($escapedItem == 'couleur'){
                        $escapedItem = 'category';
                    }
            ...
            try {
                ...
                \DB::transaction(function () use ($cleanedData , $warehouses,&$errors) {
                    ...
                    foreach ($cleanedData as $key => $value) {
                        ...
                        $unit = Unit::where(['ShortName' => $value['unite']])
                            ->orWhere(['name' => $value['unite']])
                            ->where('deleted_at', null)->first();
                                                          

                        if(!$unit){
                            $errors[] = [
                                'unit' => 'Unit used in csv file not exist :'.$value['unite'],
                            ];
                            continue;
                        }
                        $unit_id = $unit->id;
                        ...
                        if ($value['marque'] != 'N/A' && $value['marque'] != '') {
                            $brand = Brand::where('deleted_at', null)->firstOrCreate(['name' => $value['marque']]);
                            $brand_id = $brand->id;
                        } else {
                            $brand_id = NULL;
                        }
                        ...
                        $Product = new Product;
                        $Product->name = htmlspecialchars(trim($value['dessin']));;
                        $Product->code = $this->generate_random_code('');
                        $Product->Type_barcode = 'CODE128';
                        $Product->type = 'is_single';
                        $Product->price = str_replace(",","",$value['prixvente']);
                        $Product->cost = str_replace(",","",$value['prixachat']);
                        $Product->category_id = $category_id;
                        $Product->brand_id = $brand_id;
                        $Product->TaxNet = 0;
                        $Product->tax_method = 1;
                        $Product->note = $value['remarque'] ? $value['remarque'] : '';
    ...
    public function generate_random_code($value_code)
    {
        if($value_code == ''){
            ...
            return $this->check_code_exist($gen_code);
        }else{
            return $this->check_code_exist($value_code);
        }
    }
